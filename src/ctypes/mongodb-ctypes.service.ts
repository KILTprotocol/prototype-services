...
export class MongoDbCTypesService implements CTypeService {
  ...
  public async findAll(): Promise<CType[]> {
    const result: CTypeDB[] = await this.cTypeDBModel.find()
                                        .exec()
    return result.map(
      (cTypeDB: CTypeDB): CType => convertToCType(cTypeDB)
    )
  ...
  public async removeAll(): Promise<void> {
    await this.cTypeDBModel.deleteMany({})
              .exec()
  }

  private async _findByHash(hash: sdk.ICType['hash']): Promise<Optional<CTypeDB>> {
    const result: CTypeDB = await this.cTypeDBModel
                                      .findOne({'hash': hash})
                                      .exec()
