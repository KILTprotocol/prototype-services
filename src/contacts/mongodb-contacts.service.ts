...
export class MongoDbMContactsService implements ContactsService {
  ...
  public async add(contact: Contact): Promise<void> {
    const modifiedContact: ContactDB = (await this._findByAddress(contact.publicIdentity.address)).orElse(
      new this.contactModel(contact as ContactDB)
    )
  ...
  public async findByAddress(address: string): Promise<Optional<Contact>>{
    ...
    return result.map((contact: ContactDB): Contact => convertToContact(contact))
  }

  public async list(): Promise<Contact[]> {
    const result: ContactDB[] = await this.contactModel.find()
                     .exec()
    return result.map((contact: ContactDB): Contact => convertToContact(contact))
  }
  ...
  private async _findByAddress(address: string): Promise<Optional<ContactDB>> {
    const result: ContactDB = await this.contactModel.findOne({'publicIdentity.address': address})
                                        .exec()
